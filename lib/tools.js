...
function parseXSenseOutput(rawOutput, knownSerials = []) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (deviceHeaderMatch) {
            ...
            const rawName = normalizeString(deviceHeaderMatch[1].replace(/�/g, 'ä'));
            ...
            if (key === 'serial') {
                ...
                pairs.forEach(pair => {
                    ...
                    if (k && vParts.length > 0) {
                        k = normalizeString(k.trim().replace(/^'|'$/g, ''));
                        let v = normalizeString(vParts.join(':').trim().replace(/^'|'$/g, ''));
                        if (k === 'online') {
