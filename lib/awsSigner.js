...
class AWSSigner {
     ...
     */
    _getSigningKey(dateStamp, region) {
        const kDate    = this._sign(Buffer.from(`AWS4${this.secretAccessKey}`, 'utf-8'), dateStamp);
        const kRegion  = this._sign(kDate, region);
        const kService = this._sign(kRegion, this.service);
    ...
    signHeaders(method, url, region, extraHeaders, content) {
        ...
        const { amzDate, dateStamp } = this._timestamps();
        ...
        if (content) {
            ...
            contentHash = this._sha256Hex(Buffer.from(contentStr, 'utf-8'));
        } else {
            contentHash = this._sha256Hex('');
        }
        ...
        // Signatur
        const signingKey  = this._getSigningKey(dateStamp, region);
    ...
    presignUrl(url, region) {
        ...
        const contentHash     = this._sha256Hex(''); // leer für WebSocket
