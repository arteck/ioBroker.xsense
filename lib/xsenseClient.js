...
const PROPERTY_MAP = {
    '*':      { wifiRssi: 'wifiRSSI' },
    STH0A:  { a: 'alarmStatus', b: 'temperature', c: 'humidity', d: 'temperatureUnit', e: 'temperatureRange', f: 'humidityRange', g: 'alarmEnabled', h: 'continuedAlarm', t: 'time' },
    STH51:  { a: 'alarmStatus', b: 'temperature', c: 'humidity', d: 'temperatureUnit', e: 'temperatureRange', f: 'humidityRange', g: 'alarmEnabled', h: 'continuedAlarm', t: 'time' },
};

const TYPE_MAPPERS = {
    batInfo:        v => batInfoToPercent(parseInt(v, 10)),
...
class XSenseClient {
    ...
    async apiCall(bizCode, unauth = false, params = {}) {
        if (!unauth && this._isAccessTokenExpiring()) {
            await this.refresh();
        }
            ...
            try {
                ...
                for (const s of (stationsData?.stations || [])) {
                    ...
                    for (const d of (s.devices || [])) {
                        const devType = d.category || d.deviceType || d.type || d.devType || '';
                        if (!devType) {
                            this.log.warn(`[XSense] Gerät ${d.deviceSn || d.deviceId} hat keinen Typ – API-Felder: ${JSON.stringify(Object.keys(d))}`);
                            this.log.debug(`[XSense] Rohdaten Gerät: ${JSON.stringify(d)}`);
                        }
    ...
    async getThingShadow(station, page) {
        if (this._isAwsTokenExpiring()) {
            await this.loadAws();
        }

        let typeName = station.type || '';
        if (typeName === 'SBS10' || typeName === 'SBS50') {
            typeName = '';
        }
        ...
        const host = `${station.house.mqttRegion}.x-sense-iot.com`;
        ...
        const signedExtra = this.signer.signHeaders('GET', url, station.house.mqttRegion, baseHeaders, null);
        ...
        try {
            ...
            throw new Error(`[XSense IoT] Netzwerkfehler bei Shadow ${page}: ${err.message || err.code || String(err)}`);
        }
    ...
    async getState(station) {
        ...
        if (reported) {
            this._parseReported(station, reported);
        }
    }
     ...
     */
    _parseReported(station, reported) {
        ...
        if (reported.devs && typeof reported.devs === 'object') {
            for (const [sn, devData] of Object.entries(reported.devs)) {
                const device = this._findDeviceBySn(station, sn);
                if (device) {
                    this._applyDeviceData(device, devData);
                }
            }
            return;
        }
...
continue;
            ...
            const device = this._findDeviceBySn(station, key);
            if (device) {
                this._applyDeviceData(device, value);
            }
        }
    }
     ...
     */
    _applyDeviceData(device, rawData) {
        const data = { ...rawData };
        ...
        // status-Substruktur einfalten
        const statusFields = data.status || {};
        delete data.status;

        // software version normalisieren (swMain → sw)
        if ('swMain' in data) {
    ...
    async doThingShadow(station, topic, data) {
        ...
        const signedExtra = this.signer.signHeaders('POST', url, station.house.mqttRegion, baseHeaders, bodyStr);
        const headers     = { ...baseHeaders, ...signedExtra };
            ...
            for (const station of Object.values(house.stations)) {
                for (const device of Object.values(station.devices)) {
                    if (device.serial === deviceSerial) {
                        return await this._executeAction(station, device, 'test');
                    }
                }
            }
        }
     ...
     * @returns {Promise<string>}  Statusmeldung nach erfolgreicher Ausführung
     */
    async _executeAction(station, device, action) {
...
flat[k] = v;
     ...
     *
     * @param {object}  house    Haus-Objekt mit { houseId }
     * @param {object}  station  Station-Objekt mit { serial }
     * @returns {string[]}       Array der zu subscribenden MQTT-Topics
     */
    getMqttTopics(house, station) {
        return [
            `@xsense/events/+/${house.houseId}`,
            `$aws/things/${house.houseId}/shadow/name/+/update`,
            `$aws/things/${station.serial}/shadow/name/+/update`,
            // get/accepted: Antwort auf Shadow-GET-Anfragen beim Start
            `$aws/things/${station.serial}/shadow/name/+/get/accepted`,
...
return null;
        ...
        // Station anhand stationSN finden – bei get/accepted fehlt stationSN im Payload,
        // dann Serial aus Topic ableiten: $aws/things/<serial>/shadow/name/...
        let stationSn = stationData.stationSN;
        if (!stationSn) {
            const topicMatch = topic.match(/\$aws\/things\/([^/]+)\/shadow/);
            stationSn = topicMatch?.[1] ?? null;
        }
...
return null;
        ...
        const station = this._findStationBySn(stationSn);
        if (!station) {
return null;
}
        ...
        this._parseReported(station, stationReported);
...
module.exports = { XSenseClient, batInfoToPercent, rfLevelToString, STATE_SIGNAL, DEFAULT_SCAN_INTERVAL_S, REALTIME_DEVICE_TYPES };
