...
class DeviceController extends dmUtils.DeviceManagement {
    ...
    buildDeviceTile(device, devicePath, station, houseName) {
        ...
        if (['XC01-M', 'XC04-WX', 'SC06-WX', 'SC07-MR', 'SC07-WX'].includes(type) && data.coPpm !== undefined) {
            items.coPpm = {
        ...
        return {
            ...
            manufacturer: 'X-Sense',
            model: getModelDescription(type),
            status,
            hasDetails: true,
            customInfo: Object.keys(items).length > 0
    ...
    buildDeviceDetails(device, station, stationPath) {
        ...
        const infoItems = {
            ...
            type:   { type: 'staticInfo', label: 'Typ',           data: getModelDescription(type),      size: 16, addColon: true, newLine: true },
    ...
    buildDeviceActions(device) {
        ...
        // device.type bevorzugt – Fallback auf device.data?.type falls device.type leer
        const type = device.type || device.data?.type || '';
        if (!type || !TESTABLE.includes(type)) { return []; }
