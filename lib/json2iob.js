...
class Json2iobXSense {
    ...
    async parseHouses(basePath, houses) {
        ...
        for (const house of Object.values(houses)) {
            ...
            await this._writeFields(housePath, {
            ...
            for (const station of Object.values(house.stations)) {
                ...
                await this._ensureDevice(basePath, bridgePath, station.name || station.serial);
                ...
                await this._writeFields(bridgePath, stationFields);
                ...
                for (const device of Object.values(station.devices)) {
                    ...
                    await this._ensureChannel(basePath, devicePath, device.name || device.serial);
                    ...
                    await this._writeFields(devicePath, deviceFields);
                }
            }
        }
    }
     ...
     * @param {string} name
     */
    async _ensureDevice(basePath, id, name = '') {
     ...
     * @param {string} name
     */
    async _ensureChannel(basePath, id, name = '') {
        await this.adapter.setObjectNotExistsAsync(id, {
            type: 'channel',
            common: {
                name: name || id.split('.').pop(),
     ...
     * @param {object} fields
     */
    async _writeFields(basePath, fields) {
        for (const [key, value] of Object.entries(fields)) {
            if (value === null || value === undefined) {
                continue;
            }

            // 'type' ändert sich nie – wenn der neue Wert leer ist,
            // den bestehenden ioBroker-Datenpunkt nicht überschreiben
            if (key === 'type' && (value === '' || value === undefined)) {
                const id       = `${basePath}.${await this.name2id(key)}`;
                const existing = await this.adapter.getStateAsync(id);
                if (existing?.val) {
                    continue; // bestehender Wert bleibt erhalten
                }
            }
    ...
    async setStateObject(id, value) {
        ...
        if (lastPart === 'batInfo' && typeof value === 'number') {
            // value ist bereits Prozent (0–100), umgerechnet durch TYPE_MAPPERS in xsenseClient
            type  = 'number';
            role  = 'value.battery';
            unit  = '%';
            min   = 0;
            ...
            switch (true) {
                ...
                case lastPart === 'temperature':
                    role = 'value.temperature'; unit = '°C'; type = 'number';
                    break;
                case lastPart === 'humidity':
                    role = 'value.humidity'; unit = '%'; type = 'number';
                    break;
                case lastPart === 'coPpm':
                    role = 'value'; unit = 'ppm'; type = 'number';
                    break;
                case lastPart === 'alarmVol' || lastPart === 'voiceVol':
                    role = 'level.volume'; unit = '%'; type = 'number';
                    break;
                case lastPart === 'isLifeEnd':
                    role = 'indicator'; type = 'boolean';
                    value = value === 1 || value === '1' || value === true;
                    break;
                case lastPart === 'isOpen':
                    role = 'sensor.door'; type = 'boolean';
                    value = value === '1' || value === 1 || value === true;
                    break;
                ...
                case lastPart === 'continuedAlarm':
                    role = 'indicator.alarm'; type = 'boolean';
                    break;
...
common.max = max;
        ...
        if (setAlarm) {
            ...
            await this.adapter.setObjectNotExistsAsync(msgId, {
                type: 'state',
                common: { name: 'answer test_Alarm_Message', type: 'string', role: 'text', read: true, write: false, def: '' },
                native: {},
            });
            // Nur schreiben wenn noch kein Wert vorhanden
            const existing = await this.adapter.getStateAsync(msgId);
            if (existing === null || existing === undefined) {
                await this.adapter.setStateAsync(msgId, { val: '', ack: true });
            }

            await this.adapter.setObjectNotExistsAsync(alarmId, {
                type: 'state',
                common: { name: 'Toggle state test_Alarm', type: 'boolean', role: 'button', read: true, write: true, def: false },
                native: {},
            });
            ...
            if (alarmState === null || alarmState === undefined) {
                await this.adapter.setStateAsync(alarmId, { val: false, ack: true });
            }

            // Einmalig subscriben
            this._subscribeOnce(alarmId);
        }
    }
    ...
    async createStaticDeviceObject() {
        ...
        this._subscribeOnce('devices.forceRefresh');
    }
    ...
    async parse(basePath, obj) {
        ...
        for (const key of Object.keys(devicesContainer)) {
            ...
            if (Object.prototype.hasOwnProperty.call(device, 'wifiRSSI')) {
                ...
                await this._ensureDevice(basePath, await this.name2id(targetPath), await this.name2id(deviceName));
...
continue;
                ...
                deviceName = typeof device.name === 'string' ? device.name : '';
                await this._ensureChannel(basePath, await this.name2id(targetPath), await this.name2id(deviceName));
                await this.parseObject(targetPath, device);
            }
        }
    }
